...
def login_kontol():
	banner()
	print("""%s ````````````````````````````````````````````````````````````` """%(h))
	print("""%s    \33[1;32حمادي🇱🇾 """%(h))
	print("""%s ````````````````````````````````````````````````````````````` """%(h))
	panda = input(x+'\33[1;96m•اكتب توكن> ')
	...
	try:
		...
		print("""%s \n"""%(h))
		print("""%s ● """%(h))
		jalan('%s•••••|•••••√[%s✓%s] %s\33[1;96mتم حفظ التوكن روح شغل الاداة من جديد ❤️'%(M,P,M,P))
		print("""%s ● """%(h))
		print("""%s \33[1;96mالمبرمج حمادي🇱🇾 : قناتي @LY_Sk_M4 : معرفي @SK_M2 """%(h))
		time.sleep(2.5)
		exit()
	except KeyError:
		print("""%s \n"""%(h))
		jalan('%s•••••|•••••√[%s!%s] %sالتوكن متعطل روح جيب غيرو 💔'%(M,P,M,P))
		time.sleep(2.5)
		login_kontol()
	except requests.exceptions.ConnectionError:
...
def menu(my_name):
	banner()
	print(x+'\33[m[~_~]\033[96mاسم الحساب ميستخرج منه توكن 😁🧷  : \33[m'+str(my_name))
	print("""%s ````````````````````````````````````````````````````````````` """%(h))
	print("""%s \33[m[01] \33[1;33mالصيد من الاصدقاء """%(h))
	print("""%s \33[m[00] \33[mتسجيل خروج"""%(h))
	print("""%s ````````````````````````````````````````````````````````````` """%(h))
	jh = input(x+'\33[m[?] \33[1;96mInput Number : اختيارك?> ')
	if jh in ['1','01']:
		dump_massal()
	elif jh in ['2','02']:
		result()
...
def dump_massal():
	clear()
	banner()
	print("""%s  \33[m[~_~]\33[1;32mكيفك 😂❤️\33[m[~_~]"""%(h))
	print("""%s ● """%(h))
	print(x+'\33[m[ ! ] \33[1;96m المبرمج : حمادي🇱🇾 [ @SK_M2 ]')
	try:
		jum = int(input(x+'\33[m[ ? ] \33[1;96m[ID] عدد الايديات ? '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input(x+'\33[m[ ? ] \33[1;96mاكتب الايدي '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def setting():
	print("""%s ● """%(h))
	print("""%s \33[m[01] \33[1;32mقديم (الافضل)"""%(h))
	print("""%s \33[m[02] \33[1;32mجديد """%(h))
	print("""%s ● """%(h))
	hu = input(x+'\33[m[ ? ] \33[1;96mInput Number : اختيارك?: ')
	if hu in ['1','01']:
		for bacot in id:
			id2.append(bacot)
	elif hu in ['2','02']:
	...
	print("""%s ● """%(h))
	print("""%s \33[1;32m[01] B-API : ايبي """%(h))
	print("""%s ● """%(h))
	hc = input(x+'\33[m[ ? ] \33[1;96m•Input Number : اختيارك?>  ')
	if hc in ['1','01']:
	...
	print("""%s ````````````````````````````````````````````````````````````` """%(h))
	aplik = input(x+'\33[m[ ? ] \33[1;32m[T]اكتب حرف[T]  ? (t):  ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	osk = input(x+'\33[m[ ? ] \33[1;33m•[T]اكتب حرف[T] ? (t):  ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
def passwrd():
	...
	banner()
	print("""%s  """%(h))
	print("""%s\33[m_   _    _       __   __
| | | |  / \  |  \/  |  _ \ \ / /
| |_| | / _ \ | |\/| | | | \ V /      \033[31;1m حمادي 
|  _  |/ ___ \| |  | | |_| || |
|_| |_/_/   \_\_|  |_|____/ |_|  \033[1;34mاحبك"""%(h,okc,cpc))
	print("""%s  """%(h))
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [HMADY🇱🇾] %s/%s ❤️ [OK:%s] 🧡 [CP:%s] 🥺 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack3(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s [HMADY🇱🇾] %s/%s ~_~ [OK:%s | CP:%s] ~_~ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
